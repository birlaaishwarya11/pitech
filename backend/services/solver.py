...
from backend.models.schemas import GroupedStop, VehicleRecord
...
def solve_vrp(
    ...
    """
    # Use dynamic parameters if provided, otherwise fall back to static config
    if params is None:
    ...
    routing.AddDimension(
        ...
        params.solver_max_waiting_minutes,       # max waiting time (slack)
    ...
    for v in range(num_vehicles):
        ...
        v_start = vehicle_start_times[v] if vehicle_start_times else params.depot_open_minutes
        time_dimension.CumulVar(start_index).SetRange(v_start, params.depot_close_minutes)
        time_dimension.CumulVar(end_index).SetRange(v_start, params.depot_close_minutes)
    ...
    for stop_idx in range(num_stops):
        ...
        routing.AddDisjunction(
            [manager.NodeToIndex(node)], params.drop_penalty
        )
    ...
    limit = time_limit_seconds if time_limit_seconds is not None else params.solver_time_limit_seconds
