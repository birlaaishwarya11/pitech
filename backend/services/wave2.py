...
def plan_second_wave(
    ...
    for v_id in range(len(vehicles)):
        ...
        if route_stops:
            ...
            wave2_start = (
                ...
                + params.wave2_reload_buffer_minutes
            )
        else:
            # Idle in wave 1 — available immediately
            wave2_start = params.depot_open_minutes
    ...
    eligible: list[tuple[int, int]] = sorted(
        (
            ...
            if start_t <= params.wave2_cutoff_minutes
        ),
        key=lambda x: x[0],
    )
    ...
    w2_result = solve_vrp(
        ...
        time_limit_seconds=params.wave2_solver_time_limit_seconds,
        params=params,
    )
